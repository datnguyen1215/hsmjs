...
describe("Transition Guards", () => {
  describe("Basic Guard Conditions", () => {
    ...
    beforeEach(() => {
      ...
      idle
        .on("START", active)
        .if((ctx) => ctx.canStart === true);

      idle
        .on("START", error)
        .if((ctx) => ctx.canStart === false);
  ...
  describe("Multiple Guards Priority", () => {
    ...
    beforeEach(() => {
      ...
      idle
        .on("GO", stateA)
        .if((ctx) => ctx.priority === 1);

      idle
        .on("GO", stateB)
        .if((ctx) => ctx.priority === 2);

      idle
        .on("GO", stateC); // No guard - fallback
